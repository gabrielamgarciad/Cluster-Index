...
import skimage.util # -------- for image montages (makeSlide)
import concurrent.futures # -- for parallel processing
import logging

def makeSlide(A):
    ''' displays 3D array as a 2D grayscale image montage'''
    plt.imshow(skimage.util.montage([abs(A[:,:,k]) for k in range(0,A.shape[2])], padding_width=1, fill=0),cmap='jet')
    plt.show()
...
def calculate_CV(defectArrayShape,activeVoxel,defVec,spherePx):
    ...
    return np.append(activeVoxel,spherePx[ii,0])

def calculate_CI(defectArray,vox,Rmax=50,type='fast'):
...
# -- import some data and create the defectArray -- ##
#nii_data, _, nii_hdr = load_Nifti_file('C:/PIRL/GUI/Niftis/MuHe437.nii.nii.gz')
#defectArray = nii_data[:,:,:,1]

# -- or, load a numpy array for calculation -- ##
noisyCircles = np.load('C:/Pirl/R/noisyCircles.npy')
defectArray = noisyCircles[:,:,:,0,7,0]
makeSlide(defectArray)

# -- ...or, create some test defectArray. Here's a sphere -- ##
#x,y,z = np.meshgrid(np.arange(-20,20,1),np.arange(-20,20,1),np.arange(-20,20,1))
#defectArray = (x**2 + y**2 + z**2)<15**2

CI = calculate_CI(defectArray,[3.9,3.9,3.9])
makeSlide(CI)
